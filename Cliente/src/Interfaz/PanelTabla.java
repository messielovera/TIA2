...
public class PanelTabla extends JPanel{
	...
	public void actualizar(){
		NormalDistribution nd = new NormalDistribution(100,100); // Crea la distribuci�n. Debe colocarse en la sesi�n del ADMIN!
		double demanda = nd.sample(); // Obtiene un dato de la distribuci�n. Debe colocarse en la sesi�n del ADMIN! se hace broadcast del numero al "terminar" la ronda
		double producido = Double.parseDouble(tablaResultados[5][ronda].getText()); //Lo que el usuario coloca
		double inventario = Double.parseDouble(tablaResultados[6][ronda].getText()) + producido - demanda; //Inventario (t) = lo que ten�a + lo q entra - lo q sale 
